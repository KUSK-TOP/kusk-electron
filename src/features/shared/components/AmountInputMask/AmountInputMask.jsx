...
import { parseKUSKAmount, formatKUSKAmount, addZeroToDecimalPosition } from 'utility/buildInOutDisplay'
  ...
  componentDidMount() {
    if(this.props.fieldProps.value){
      ...
      this.props.fieldProps.onChange(
        parseKUSKAmount(value, this.props.decimal )
      )
      ...
      <div className='form-group'>
        {this.props.title && <FieldLabel>{this.props.title}</FieldLabel>}
          <input className='form-control'
            type={'text'}
            {...fieldProps}
            value={formatKUSKAmount(this.state.value, this.props.decimal)}
