...
import { normalizeGlobalKUSKAmount , converIntToDec } from 'utility/buildInOutDisplay'
...
class Modal extends React.Component {
  ...
  normalize(inouts) {
    ...
    inouts.forEach(inout => {
      ...
      if(filteredAsset.length === 1){
        ...
        amount = assetId===kuskID?
          normalizeGlobalKUSKAmount(kuskID, amount, this.props.kuskAmountUnit):
  ...
  render() {
    ...
    let view = <div></div>
    if (item.length !== 0) {
      const fee = normalizeGlobalKUSKAmount(kuskID, item.fee, kuskAmountUnit)
