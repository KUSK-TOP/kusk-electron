...
export const balance = (values, assetDecimal, balances, kuskAmountUnit) => {
  ...
  if(filteredBalances.length === 1){
    if (filteredBalances[0].assetId === kuskID){
      return normalizeKUSKAmountUnit(filteredBalances[0].assetId, filteredBalances[0].amount, kuskAmountUnit)
...
export const getAssetDecimal = (values, asset) => {
  ...
  return (filteredAsset.length === 1 && filteredAsset[0].definition && filteredAsset[0].id !== kuskID ) ?
